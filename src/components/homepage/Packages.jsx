...
const Packages = () => {
  ...
  return (
    <div className='py-8 md:py-16 bg-gradient-to-b from-[#f4f6ff] to-[#ffffff] poppins px-4 md:px-0'>
      <Container>
        <div className="flex flex-col w-full justify-center items-center gap-1 md:gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:mx-20 md:mt-10 mt-5  px-4 md:px-0">
          {plansData.map((plan) => (
            <Link to={plan.url}>
            <div className={`flex flex-col gap-1 md:gap-3 col-span-1 border border-[#E7EBFF] rounded-lg shadow-xl shadow-[#BCCAFF21] p-5 bg-no-repeat hover:bg-pattern transition-colors ease-in bg-cover bg-white `}
              ...
              <div className="flex flex-col">
                <h3 className='md:text-2xl font-bold text-lg playfair-display-font'>{plan.title}</h3>
                <h4 className={` hover:text-black text-xs md:text-sm font-light w-xs mt-[-2px] md:mt-0`}>{plan.description}</h4>
              </div>
              <h4 className='text-lg md:text-2xl font-normal flex gap-1 md:gap-2 items-center border-b md:pb-3 pb-2 mb-2'>{plan.monthly} <span className='text-gray-700 text-xs md:text-sm font-light'>per month</span></h4>
              <div className="flex aspect-[21/9] md:aspect-[16/9]   border-white rounded-lg overflow-hidden relative">
                ...
                >
                  <Tooltip title="Play" placement="bottom">
                    <Play color="#253359" className="md:h-[18px] h-[20px]" fill="#253359" />
                  </Tooltip>
                </button>
              </div>
              ...
              <Link to={plan.url}>
                <button className='w-full border border-black rounded-lg md:py-2 py-1 transition-colors hover:text-[#2e4a96] hover:border-[#2e4a96] text-sm md:text-base'>Get Started</button>
              </Link>
            </div>
            </Link>
          ))}
